...
#define _RGB_CAPSLOCK_ENABLED_COLOR  0xFF, 0x00, 0x00
#define _RGB_CAPSLOCK_DISABLED_COLOR 0x03, 0xDB, 0xFC
...
//      Ct_L     Win_L    Alt_L                               SPACE                               Alt_R    FN       Ct_R     Left     Down     Right
    ...
    [1] = LAYOUT(
        _______, _______, _______, _______, _______, _______, _______, KC_MPRV, KC_MSTP, KC_MNXT, KC_MUTE, KC_VOLD, KC_VOLU, _______,          _______,
...
void rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    int side_leds[] = { 
      67, 70, 73, 76, 80, 83, 87, 91, // Left
      68, 71, 74, 77, 81, 84, 88, 92  // Right
    };

    if (host_keyboard_led_state().caps_lock) {
      for (int i=0; i < (sizeof(side_leds)/sizeof(side_leds[0])); i++) {
        rgb_matrix_set_color(side_leds[i], _RGB_CAPSLOCK_ENABLED_COLOR);
      }
    } else {
      for (int i=0; i < (sizeof(side_leds)/sizeof(side_leds[0])); i++) {
        rgb_matrix_set_color(side_leds[i], _RGB_CAPSLOCK_DISABLED_COLOR);
      }
    }
